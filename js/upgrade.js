...
(function () {
  ...
  var DATA_VERSION = '2026.09.09.01';
  ...
  /* 移动端长按顶栏图标弹出功能提示（借按钮 title）；PC 端 hover 已由原生 title 提供 */
  function patchLongPressHint() {
    try {
      var timer = null, fired = false;
      function cancel() { if (timer) { clearTimeout(timer); timer = null; } }
      document.addEventListener('touchstart', function (e) {
        cancel();
        var t = (e.target && e.target.closest) ? e.target.closest('button[title]') : null;
        if (!t) return;
        fired = false;
        timer = setTimeout(function () { fired = true; try { toast(t.getAttribute('title') || ''); } catch (x) {} }, 500);
      }, { passive: true });
  ...
  /* 运行时按当前年份填充 data-qx-year 节点，杜绝页面写死年份 */
  function qxFillYears() {
    try {
      var y = String(new Date().getFullYear());
      Array.prototype.forEach.call(document.querySelectorAll('[data-qx-year]'), function (n) { n.textContent = y; });
    } catch (e) {}
  }
  ...
  function qxRequestPersist() {
    try {
      ...
      if (navigator.storage && typeof navigator.storage.persisted === 'function') {
        navigator.storage.persisted().catch(function () {});
      }
    } catch (e) {}
  }

  /* 全局异常捕获：绝不白屏。记录最近错误到进程内缓冲，供「导出故障快照」读取；
     重要错误仅作一次非打扰提示，避免刷屏。故障快照只保存到本机，绝不上传。 */
  var QX_ERRS = [];
  function patchGlobalErrors() {
    function push(m) {
      try { QX_ERRS.push({ t: new Date().toISOString(), m: String(m || '').slice(0, 400) }); if (QX_ERRS.length > 12) QX_ERRS.shift(); } catch (e) {}
    }
    window.addEventListener('error', function (ev) {
      push(ev && ev.message);
      if (ev && ev.message && /quota|securityerror|denied|not(enough| allowed)|quotaexceeded/i.test(ev.message)) {
        try { toast('保存受限：请检查浏览器隐私设置或存储空间，重要数据建议导出备份', 'warn'); } catch (e2) {}
      }
    });
    window.addEventListener('unhandledrejection', function (ev) {
      push(ev && ev.reason && (ev.reason.message || ev.reason));
    });
  }
  function supportSnapshot() {
    var data = {
      ...
      page: document.title || '', hash: (location.hash || ''),
      storedApprox: fmtBytes(storeSize()),
      recent: QX_ERRS
    };
    try {
      ...
      var url = URL.createObjectURL(blob);
      var a = document.createElement('a');
      a.href = url; a.download = '栖匣-故障快照-' + todayKey() + '.json';
  ...
  /* 在「更新日志 → 版本自检」卡片下方注入「导出故障快照」入口 */
  function injectSnapshotEntry() {
    try {
      var rep = document.getElementById('qxSelfReport');
      if (!rep || rep.getAttribute('data-qxsnap')) return;
