...
- **产品**：栖匣 —— 大学生成长中枢，一套可跨端的产品：网页即开（PWA 可装到桌面/主屏），并规划了桌面与 Android 原生 App；数据默认加密保存在本机，可导出备份，也可选配 WebDAV（如坚果云）或自托管服务端实现多端同步。**不要**在面向用户的文案中出现「纯前端」「零安装」「只存本机/本地」「不上传服务器」等与"多端 App + 云同步"定位冲突的说法。
...
- 默认无后端、不上传（数据默认存本机），隐私声明与免责小字不可删（涉及教育培训、竞赛、理财、论文诚信、副业防骗等板块）；云同步为**用户手动配置**的可选项。
