...
/* v35 · 缓存版本 v46：新增四大功能（一键成长报告导出、过去日期补卡、番茄钟→习惯联动计数、"期末冲刺"倒数预设）+ 数据备份全面升级——导出/导入/清空改为「全量快照」。 */
/* v36 · 缓存版本 v47：移除「情商与社交」冗余板块（占位且已合并），清理其死代码引用；更新说明改写为「从第一版到现在的成长」用户向变更历程并删除面向作者的「体验与安全」内部段落；产品导览收尾区彻底去除红色印章与“书”的隐喻（产品非书），改回品牌青绿留白风格并保留「把四年，过出掌控感」唯一口号。 */
/* v37 · 缓存版本 v48：奖助福利板块重写——新增「家庭经济困难认定」「绿色通道」两张标准卡片（什么人适合/为什么/能得到什么/如何申请/诚信红线，给真正需要的人，去掉易生歧义的表述）；更新说明收尾去夺金化；新增「自动保存+每日自动备份+一键恢复」（保留7天，防误清空丢数据）；成长报告改为页内预览不下载、置于首页一键查看；返回时保持原板块与滚动位置。 */
/* v38 · 缓存版本 v49：成长报告移至「系统设置」顶部、复制粘贴去掉大段横线边框更精简；首次开屏时间适当延长（3.6s 看清品牌）后产品导览自动出现、需手动关闭；板块「奖助福利」更名「奖助与权益」，重写开篇说明与「从哪个开始」引导表，删除「应得尽得/不吃亏/到手多少/漏领」等易生歧义表述。 */
/* v39 · 缓存版本 v50：板块更名「助学帮扶」(4字，边栏/导览/更新说明全同步)；成长报告移到「成长档案」板块顶部、点开即按最新数据实时汇总并显示更新时间；修复打印/另存为PDF（改为新窗口调打印，微信内给出引导）；复制全文/打印/关闭/查看按钮均给即时反馈；替换「血泪/崩溃」等夸张用词为平和准确表述；备份按钮点击后提示备份到本机存储及数据项数与大小；首开沿用 3.6s 品牌屏＋导览手动关闭。 */
const CACHE = 'pingwanxiang-v50';
